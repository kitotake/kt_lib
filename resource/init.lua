...
lib = setmetatable({
    ...
    __newindex = function(self, key, fn)
        ...
        if debug_getinfo(2, 'S').short_src:find('@kt_lib/resource') then
            exports(key, fn)
        end
    end,
    ...
    __index = function(self, key)
        ...
        if chunk then
            local fn, err = load(chunk, ('@@kt_lib/%s/%s.lua'):format(key, self.context))
...
if not LoadResourceFile(lib.name, 'web/build/index.html') then
    local err =
    '^1Unable to load UI. Build kt_lib or download the latest release.\n	^3https://github.com/kitotake/kt_lib/releases/latest/download/kt_lib.zip^0'
